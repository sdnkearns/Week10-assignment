...
data input files:  
&ensp;  aan520_w5a_actors2000After.csv  
&ensp;  aan520_w5a_genres2000After.csv  
&ensp;  aan520_w5a_movies2000After.csv  
&ensp;  aan520_w5a_roles2000After.csv  

create_imdb_duckdb.py  
&ensp;  -python script that reads the input data csv files, puts the data into a polars dataframe, then stores that data in a DuckDB database

imdb.duckdb  
&ensp;  -DuckDB database created by the python script

go.mod, go.sum, imdb_query.go  
&ensp;  -go project files for accessing the imdb DuckDB database, and executing some example queries

main_test.go  
&ensp;  -contains unit tests for imdb_query.go

create duckdb database as:  
&ensp;  python create_imdb_database.py

run go query as:  
&ensp;  go run imdb_query.go

build go query as:  
&ensp;  go build imdb_query.go
